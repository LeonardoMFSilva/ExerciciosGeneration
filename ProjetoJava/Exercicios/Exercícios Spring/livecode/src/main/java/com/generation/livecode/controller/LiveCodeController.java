...
public class LiveCodeController {
	...
	/*Chamando o metho (query annotation)*/
	...
	@PostMapping
	public ResponseEntity<LiveCode> postCategoria(@RequestBody LiveCode titulo) {
		return ResponseEntity.status(HttpStatus.CREATED).body(repository.save(titulo));
	}

	@PutMapping
	public ResponseEntity<LiveCode> putCategoria(@RequestBody LiveCode titulo) {
